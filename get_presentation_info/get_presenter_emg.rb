...
if flag.nil?
    begin
        ...
        puts "--ERROR-- \n1. 対応するHPのHTMLをローカルに保存(ファイル名はNotionの実施月と同様とし、get_presenterと同じフォルダに保存してください)\n2.「get_presenter.rb 1」で再度実行してください"
        exit 
    end
elsif flag == "1"
    presentation_data = NokogiriData.new(HP_html.new(nil, month + ".html").nokogiri).get_need_data
    puts "ローカルのHTMLファイルから処理を行いました"
    sleep 1
    # p e
else
    puts "存在しないフラグが入力されました"
    exit
end
